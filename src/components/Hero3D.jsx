  ...
  return (
    <div className="relative w-full h-[350px] md:h-[500px]">
      ...
      <motion.div
        ...
        className="absolute top-[-74%] md:top-[5%] right-[-2%] md:right-[-10%] z-20 max-w-[280px] md:max-w-[320px] cursor-grab active:cursor-grabbing rounded-2xl bg-linear-to-br from-[#06060C]/95 via-[#0B0B0E]/95 to-[#06060C]/95 backdrop-blur-xl border-2 border-cyan-400/40 px-4 py-3 text-sm text-slate-200 shadow-[0_20px_60px_rgba(0,255,255,0.25),0_0_40px_rgba(168,85,247,0.15)] hover:border-purple-400/50 hover:shadow-[0_20px_70px_rgba(0,255,255,0.35),0_0_50px_rgba(168,85,247,0.25)] transition-all duration-500 group"
        initial={{ opacity: 0, scale: 0.8, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={{ type: "spring", stiffness: 200, damping: 20 }}
        whileHover={{ scale: 1.03, rotate: 0.5 }}
      >
        <span className="absolute -bottom-2.5 md:-bottom-2.5 left-12 md:left-12 h-5 w-5 rotate-45 md:rotate-45 bg-linear-to-br from-[#06060C] to-[#0B0B0E] border-r-2 border-b-2 border-cyan-400/40" />

        <div className="mb-3 flex items-center gap-2 justify-between">
          <div className="flex items-center gap-2">
            <div className="relative">
              <span className="h-2 w-2 rounded-full bg-linear-to-r from-cyan-400 to-purple-500 shadow-[0_0_12px_rgba(34,211,238,0.9)] animate-pulse" />
              <span className="absolute inset-0 h-2 w-2 rounded-full bg-cyan-400 animate-ping opacity-75" />
            </div>
            <span className="text-[10px] tracking-wider uppercase font-bold bg-linear-to-r from-cyan-300 via-purple-300 to-cyan-300 bg-clip-text text-transparent">Frosty • Storytelling Mode</span>
          </div>
          <button onClick={toggleAudio} className="p-1.5 rounded-lg bg-linear-to-br from-cyan-500/20 to-purple-500/20 hover:from-cyan-500/30 hover:to-purple-500/30 border border-cyan-400/30 transition-all duration-300 hover:scale-110" title={isPlaying ? "Pause audio" : "Play audio"}>
            {isPlaying ? (
        ...
        <AnimatePresence mode="wait">
          <motion.div key={currentChapter} initial={{ opacity: 0, x: 20 }} animate={{ opacity: isTransitioning ? 0 : 1, x: isTransitioning ? -20 : 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4, ease: "easeInOut" }} className="space-y-3">
            <div className="flex items-center gap-2">
              <div className="flex-1 h-[1px] bg-linear-to-r from-transparent via-cyan-400/50 to-transparent" />
            ...
            <div className="flex items-start gap-3">
              <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${currentStory.iconBg} border-2 border-cyan-400/30 flex items-center justify-center flex-shrink-0 shadow-lg group-hover:scale-110 transition-transform duration-300`}><span className="text-2xl">{currentStory.icon}</span></div>
              <div className="flex-1">
                <h3 className="text-cyan-200 font-bold text-sm mb-1.5 leading-tight">{currentStory.title}</h3>
                <div className="relative">
                  <div className="absolute -left-2 top-0 w-1 h-full bg-linear-to-b from-purple-400/50 via-cyan-400/50 to-transparent rounded-full" />
          ...
          <div className="flex items-center gap-2">
            {storyChapters.map((_, index) => (
              <button key={index} onClick={(e) => { e.stopPropagation(); setCurrentChapter(index); }} className="relative group">
                <span className={`block h-1.5 rounded-full transition-all duration-500 ${index === currentChapter ? 'w-8 bg-linear-to-r from-cyan-400 via-purple-400 to-cyan-400 shadow-[0_0_12px_rgba(34,211,238,0.8)]' : 'w-1.5 bg-cyan-400/30 hover:bg-cyan-400/50 hover:w-4'}`} />
        ...
        <div className="mt-2 -mx-4 px-4 py-2 bg-linear-to-r from-transparent via-cyan-500/5 to-transparent border-t border-cyan-400/20 rounded-b-2xl">
